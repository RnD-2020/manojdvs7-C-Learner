...
{
	if (number <= 0 || p1 > p2 || p1<0 || p2>31 || n<0)
		return 0;

	unsigned int tbin[32] = { 0 };
	unsigned int bin[32] = { 0 };
	long int result=0, temp = number, i=0, j;
	while (temp != 0)
	{
		bin[i] = temp % 2;
		temp = temp / 2;
